...
export const Box1: FC<TempContainerProps> = ({ id }) => {
  ...
  const [image, setImage] = useState<File | string>(
    data?.image || "/tempImages/user.jpg"
  );
  const [bg, setBg] = useState<File | string>(
    data?.bg || "/tempImages/background.jpg"
  );
    ...
    <div className="w-full">
      ...
      <div className="mt-[60px] px-16 text-white">
        <input
          placeholder="標題"
          className={clsx(
            "text-[18px] bg-transparent w-fit pl-1 border border-solid",
            focus === "title" ? " border-white" : "border-transparent"
          )}
        ...
        <textarea
          ...
          ref={textareaRef}
          className={clsx(
            " box-border bg-transparent w-full pl-1 resize-none overflow-hidden border border-solid",
            focus === "content" ? " border-white" : "border-transparent"
          )}
  ...
  return (
    <div className="w-full px-16 text-white mt-2">
      <input
        placeholder="標題"
        className={clsx(
          "text-[18px] bg-transparent w-fit pl-1 mb-1 border border-solid",
          focus === "title" ? " border-white " : "border-transparent"
        )}
        onFocus={() => setFocus("title")}
        onBlur={() => setFocus("")}
      ...
      <textarea
        ...
        ref={textareaRef}
        className={clsx(
          "text-[14px] border border-solid bg-transparent w-full pl-1 resize-none overflow-hidden",
          focus === "content" ? " border-white " : "border-transparent"
        )}
        onFocus={() => setFocus("content")}
        onBlur={() => setFocus("")}
